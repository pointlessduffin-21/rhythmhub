...
class RealQueueRepository : QueueRepository {
    ...
    override fun getQueue(arcadeId: String): Flow<List<QueueItem>> = callbackFlow {
        ...
        val subscription = firestore.collection("arcades")
            .document(arcadeId)
            .collection("queue")
            ...
            .addSnapshotListener { snapshot, e ->
                ...
                if (snapshot != null) {
                    val queueItems = snapshot.documents.mapNotNull { doc ->
                        try {
                            QueueItem(
                                id = doc.id,
                                ...
                                isTwoPlayer = doc.getBoolean("isTwoPlayer") ?: false,
    ...
    override suspend fun joinQueue(arcadeId: String, username: String, isTwoPlayer: Boolean) {
        try {
            val newItem = hashMapOf(
                "userId" to "current_user", // Need real Auth ID later
                "username" to username,
                "avatarUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=$username",
                "isTwoPlayer" to isTwoPlayer,
                "timestamp" to System.currentTimeMillis()
            )
            
            // Add to subcollection
            firestore.collection("arcades")
                .document(arcadeId)
                .collection("queue")
                .add(newItem)
                .await()
                
            // Ideally also increment arcade.currentQueueSize here via specific API or Cloud Function
        } catch (e: Exception) {
    ...
    override suspend fun leaveQueue(arcadeId: String, userId: String) {
        try {
            val querySnapshot = firestore.collection("arcades")
                .document(arcadeId)
                .collection("queue")
